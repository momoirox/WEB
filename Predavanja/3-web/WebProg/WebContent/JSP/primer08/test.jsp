...
<h3>ForTokens petlja za iteriranje kroz listu stavki odvojenih proizvoljnim delimiterom</h3>
<ul>
...
<h4> Lista svih studenata</h4>

<sql:query var="studenti" sql="SELECT * FROM studenti"/>

<c:forEach var="student" items="${studenti.rows}">
<ul>
<li><c:out value="${student.ime}"/>,
<c:out value="${student.prezime}"/> </li>
</ul>
</c:forEach>

<h4> Lista svih studenata koji imaju ime Petar</h4>

<sql:query  var="studenti" sql="SELECT * FROM studenti WHERE ime=?">
  <sql:param value="Petar"/>
</sql:query>

<c:forEach var="row" items="${studenti.rows}">
<ul>
